...
export function createShareOverlay() {
  ...
  function fitThumbsToAvailableSpace() {
    const landscapeRaw = thumbsEl.style.getPropertyValue("--share-thumb-landscape");
    const portraitRaw = thumbsEl.style.getPropertyValue("--share-thumb-portrait");
    let landscape = Number.parseFloat(landscapeRaw);
    let portrait = Number.parseFloat(portraitRaw);
    if (!landscape || !portrait) return;

    let attempts = 0;
    while (attempts < 10 && (thumbsEl.scrollHeight > thumbsEl.clientHeight || thumbsEl.scrollWidth > thumbsEl.clientWidth)) {
      landscape *= 0.92;
      portrait *= 0.92;
      thumbsEl.style.setProperty("--share-thumb-landscape", `${Math.max(44, Math.round(landscape))}px`);
      thumbsEl.style.setProperty("--share-thumb-portrait", `${Math.max(44, Math.round(portrait))}px`);
      attempts += 1;
    }
  }
...
function thumbSizesForCount(count, viewportWidth = window.innerWidth) {
  ...
  return {
    ...
    portraitSize: Math.round(base.portraitSize * mobileScale)
  };
}
