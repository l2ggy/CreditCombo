...
function thumbSizesForCount(count, viewportWidth = window.innerWidth) {
  ...
  const smallScreenNeedsExtraScale = viewportWidth <= 390 && safeCount >= 4;
  if (!smallScreenNeedsExtraScale) return mobileSizes;

  return {
    landscapeSize: Math.round(mobileSizes.landscapeSize * 0.85),
    portraitSize: Math.round(mobileSizes.portraitSize * 0.85)
  };
}
