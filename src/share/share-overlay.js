...
export function createShareOverlay() {
  ...
  async function renderThumbRows(cards) {
    ...
    const { landscapeSize, portraitSize } = thumbSizesForCount(sorted.length, shareCard, thumbsEl);
...
function thumbSizesForCount(count, shareCard, thumbsContainer) {
  const safeCount = Math.max(1, Number(count) || 1);
  const cardWidth = Math.max(1, shareCard?.clientWidth || 1);
  const thumbsWidth = Math.max(1, thumbsContainer?.clientWidth || cardWidth);
  const thumbsHeight = Math.max(1, thumbsContainer?.clientHeight || Math.round(cardWidth * 0.34));

  // Internal proportions follow panel/card size.
  const targetScale = safeCount === 1
    ? 0.36
    : safeCount === 2
      ? 0.27
      : safeCount === 3
  ...
  const widthCap = safeCount <= 3
    ? thumbsWidth / safeCount
    : thumbsWidth / 3;
  const rowCount = safeCount >= 4 ? 2 : 1;
  const heightCap = thumbsHeight / rowCount;
  const landscapeSize = Math.max(34, Math.round(Math.min(cardWidth * targetScale, widthCap * 0.9, heightCap * 0.8)));
  return { landscapeSize, portraitSize: landscapeSize };
}
