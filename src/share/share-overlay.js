...
export function createShareOverlay() {
  ...
  async function renderThumbRows(cards) {
    ...
    thumbCount = sorted.length;
    syncThumbSizesToCard();
  ...
  function syncThumbSizesToCard() {
    if (thumbCount <= 0) return false;

    const next = thumbSizesForCount(thumbCount, shareCard);
    if (!next) return false;

    thumbsEl.style.setProperty("--share-thumb-landscape", `${next.landscapeSize}px`);
    thumbsEl.style.setProperty("--share-thumb-portrait", `${next.portraitSize}px`);
    return true;
  }
  ...
  function fitThumbsToAvailableSpace() {
    ...
    const cardWidth = shareCard.clientWidth;
