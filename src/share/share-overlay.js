...
export function createShareOverlay() {
  ...
  async function renderThumbRows(cards) {
    ...
    fitThumbsToAvailableSpace({ landscapeSize, portraitSize });
  }


  function fitThumbsToAvailableSpace({ landscapeSize, portraitSize }) {
    const minThumb = 36;
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      nextLandscape = Math.max(minThumb, Math.floor(nextLandscape * 0.92));
      nextPortrait = Math.max(minThumb, Math.floor(nextPortrait * 0.92));
  ...
  function open() {
    ...
    if (context?.best) renderTask = applyContext();
    dialog.focus();
  }
