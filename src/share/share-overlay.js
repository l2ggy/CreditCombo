...
export function createShareOverlay() {
  ...
  async function renderThumbRows(cards) {
    ...
    latestThumbBase = { landscapeSize, portraitSize };
    ...
    fitThumbsToAvailableSpace(latestThumbBase);
  }

  function fitThumbsToAvailableSpace(baseSizes) {
    if (!baseSizes) return false;
    if (thumbsEl.clientWidth < 2 || thumbsEl.clientHeight < 2) return false;
    ...
    const usedHeight = thumbsEl.scrollHeight;
    if (widestRow <= 0 || usedHeight <= 0) return false;

    const widthRatio = thumbsEl.clientWidth / widestRow;
    const heightRatio = thumbsEl.clientHeight / usedHeight;
    const scale = Math.min(1, widthRatio, heightRatio);
    if (scale >= 0.999) return false;

    const minThumb = 34;
    let nextLandscape = Math.max(minThumb, Math.floor(baseSizes.landscapeSize * scale * 0.98));
    let nextPortrait = Math.max(minThumb, Math.floor(baseSizes.portraitSize * scale * 0.98));
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      const overflowW = Math.max(0, ...[...thumbsEl.querySelectorAll(".shareCard__thumbRow")].map((row) => row.scrollWidth)) > thumbsEl.clientWidth;
      const overflowH = thumbsEl.scrollHeight > thumbsEl.clientHeight;
      if (!overflowW && !overflowH) break;
      nextLandscape = Math.max(minThumb, Math.floor(nextLandscape * 0.95));
      nextPortrait = Math.max(minThumb, Math.floor(nextPortrait * 0.95));
      thumbsEl.style.setProperty("--share-thumb-landscape", `${nextLandscape}px`);
      thumbsEl.style.setProperty("--share-thumb-portrait", `${nextPortrait}px`);
    }
  ...
  function open() {
    ...
    if (latestThumbBase) requestAnimationFrame(() => fitThumbsToAvailableSpace(latestThumbBase));
    dialog.focus();
  }
