...
function thumbSizesForCount(count, cardWidth = 560) {
  ...
  const normalizedCardWidth = Math.max(280, Number(cardWidth) || 560);
  const scaled = Math.round((base / 560) * normalizedCardWidth);
  const bounded = Math.max(44, scaled);

  return { landscapeSize: bounded, portraitSize: bounded };
}
