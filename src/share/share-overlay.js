...
export function createShareOverlay() {
  ...
  function fitThumbsToAvailableSpace() {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      const widthScale = widestRow > 0 ? thumbsEl.clientWidth / widestRow : 1;
      const heightScale = thumbsEl.scrollHeight > 0 ? thumbsEl.clientHeight / thumbsEl.scrollHeight : 1;
      const scale = Math.min(1, widthScale, heightScale);
      if (scale >= 0.999) break;

      nextLandscape = Math.max(minThumb, nextLandscape * scale);
      nextPortrait = Math.max(minThumb, nextPortrait * scale);
