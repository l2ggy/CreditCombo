...
function thumbSizesForCount(count, cardWidth, cardHeight) {
  const safeCount = Math.max(1, Number(count) || 1);
  const safeCardSize = Math.max(320, Math.min(Number(cardWidth) || 0, Number(cardHeight) || 0) || 520);

  const factor = safeCount === 1
    ? 0.34
    : safeCount === 2
      ? 0.24
      : safeCount === 3
  ...
  const thumbSize = Math.round(safeCardSize * factor);
  return {
    landscapeSize: Math.max(44, thumbSize),
    portraitSize: Math.max(44, thumbSize)
  };
}
