...
export function createShareOverlay() {
  ...
  async function renderThumbRows(cards) {
    ...
    const { landscapeSize, portraitSize } = thumbSizesForCount(sorted.length, window.innerWidth, rowCount);
...
function thumbSizesForCount(count, viewportWidth = window.innerWidth, rowCount = 1) {
  ...
  if (viewportWidth > 720) {
    if (rowCount <= 1) return base;
    return {
      landscapeSize: Math.round(base.landscapeSize * 0.86),
      portraitSize: Math.round(base.portraitSize * 0.86)
    };
  }
    ...
    : safeCount === 2
      ...
      : safeCount === 3
        ...
        : safeCount === 4
          ? 0.66
          : 0.64;
  const multiRowScale = rowCount > 1 ? 0.82 : 1;

  return {
    landscapeSize: Math.round(base.landscapeSize * mobileScale * multiRowScale),
    portraitSize: Math.round(base.portraitSize * mobileScale * multiRowScale)
  };
}
