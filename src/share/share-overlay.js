...
export function createShareOverlay() {
  ...
  async function renderThumbRows(cards) {
    ...
    applyThumbSizes(landscapeSize, portraitSize);
    ...
    fitThumbsToContainer(landscapeSize, portraitSize);
  }

  function applyThumbSizes(landscapeSize, portraitSize) {
  ...
  function fitThumbsToContainer(initialLandscape, initialPortrait) {
    if (!thumbsEl.clientHeight || !thumbsEl.clientWidth) return;

    const minSize = 56;
    let landscapeSize = initialLandscape;
    let portraitSize = initialPortrait;

    for (let i = 0; i < 2; i += 1) {
      const heightScale = thumbsEl.clientHeight / Math.max(1, thumbsEl.scrollHeight);
      const widthScale = thumbsEl.clientWidth / Math.max(1, thumbsEl.scrollWidth);
      const scale = Math.min(1, heightScale, widthScale);
      if (scale >= 1) return;

      landscapeSize = Math.max(minSize, Math.floor(landscapeSize * scale * 0.98));
      portraitSize = Math.max(minSize, Math.floor(portraitSize * scale * 0.98));
      applyThumbSizes(landscapeSize, portraitSize);
    }
  }
  ...
  function open() {
    ...
    if (context?.best) void renderThumbRows(context.best.combo || []);
    dialog.focus();
  }
...
function thumbSizesForCount(count, viewportWidth = window.innerWidth) {
  ...
  return {
    landscapeSize: Math.round(base.landscapeSize * 0.74),
    portraitSize: Math.round(base.portraitSize * 0.74)
  };
}
