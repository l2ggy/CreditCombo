...
function thumbSizesForCount(count, viewportWidth = window.innerWidth) {
  ...
  if (viewportWidth > 720) return desktop;

  const mobile = safeCount === 1
    ? { landscapeSize: 118, portraitSize: 118 }
    : safeCount === 2
      ? { landscapeSize: 94, portraitSize: 94 }
      : safeCount === 3
        ? { landscapeSize: 82, portraitSize: 82 }
        : safeCount === 4
          ? { landscapeSize: 80, portraitSize: 80 }
          : { landscapeSize: 74, portraitSize: 74 };

  if (viewportWidth > 420) return mobile;

  return {
    landscapeSize: Math.round(mobile.landscapeSize * 0.92),
    portraitSize: Math.round(mobile.portraitSize * 0.92)
  };
}
