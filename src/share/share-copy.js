...
export function buildShareCopy({ netValue, valuationMode, cardCount, siteHost }) {
  ...
  const heroValueLabel = `Per year after annual fees. Tailored to your spend.`;
  const support = "";
  const cta = "Find your own CreditCombo";
  const urlLabel = siteHost || "creditcombo.ca";
  const nativeShareText = `${headline} ${heroValue} per year after annual fees, tailored to this spend profile.`;
